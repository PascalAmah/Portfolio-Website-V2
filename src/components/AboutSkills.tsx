...
const SkillBadge: React.FC<{ skill: string }> = ({ skill }) => {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-zinc-800 border border-zinc-700 rounded-md text-xs text-zinc-300 hover:border-zinc-500 hover:text-white transition-colors">
      {slug && (
        <img
    ...
    <section>
      ...
      <div className="space-y-3">
        {SKILL_CATEGORIES.map((cat) => (
          <div key={cat.title}>
            <p className="text-xs text-zinc-600 mb-1.5">{cat.title}</p>
